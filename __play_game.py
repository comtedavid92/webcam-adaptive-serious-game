...
def create_events():
    # Create the events
    game_controller.create_event_dwell(TARGET_START_EVENT_ID, TARGET_START_ID, PoseLandmark.RIGHT_WRIST, 1000)
    game_controller.create_event_dwell(TARGET_END_EVENT_ID, TARGET_END_ID, PoseLandmark.RIGHT_WRIST, 500)
...
def update_data(timestamp, normalized_landmarks_and_targets):
    ...
    end_effector = normalized_landmarks_and_targets.get(PoseLandmark.RIGHT_WRIST)
...
def main():
    PoseLandmark.exclude_landmarks(POSE_EXCLUDED_LANDMARKS)
